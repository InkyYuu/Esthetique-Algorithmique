...
class AceUI:
    ...
    def render_add_evidence_to_court_record(self, evidence):
        ...
        panel = pygame.Rect(80, 90, self.w - 160, self.h - 490)
        pygame.draw.rect(self.screen, pygame.Color(250, 250, 250), panel, border_radius=6)
        pygame.draw.rect(self.screen, pygame.Color(30, 90, 150), panel, 6, border_radius=6)

        # zone image à gauche
        img_box = pygame.Rect(panel.x + 24, panel.y + 24, 230, 230)
        ...
        asset = self.get_img(img)
        if asset:
            surf = asset.get() if isinstance(asset, AnimatedGIF) else asset
            if surf:
                self._blit_scaled_center(surf, img_box, pad=8)

        # barre jaune + nom
        name_bar = pygame.Rect(img_box.right + 24, panel.y + 34, panel.w - (img_box.w + 72), 40)
        pygame.draw.rect(self.screen, pygame.Color(240, 190, 40), name_bar)

        # description (avec lignes pointillées)
        desc_rect = pygame.Rect(name_bar.x, name_bar.bottom, name_bar.w, panel.h - 110)
        self._draw_dotted_lines(desc_rect, lines=3)

        # nom
        nimg = self.font_main.render(evidence.name, True, pygame.Color(0, 0, 0))
        self.screen.blit(nimg, (name_bar.x + 12, name_bar.y + 3))
        ...
        draw_text_wrapped(
            ...
            self.font_main,
            pygame.Color(0, 0, 0),
            pygame.Rect(desc_rect.x, desc_rect.y + 13, desc_rect.w, desc_rect.h - 12),
            line_spacing=13,
        )

    def render_extra_elements(self, extraElements: List[dict[str, str]], scene: Scene):
        for elements in extraElements:
            for element, value in elements.items():
                if element == "add_evidence":
                    self.render_add_evidence_to_court_record(value)
                elif element == "evidence" or element == "profile":
                    if element == "evidence":
                        img = value.image
                    else:
                        img = value.photo
                    if scene.background == "backgrounds/defense-side.jpg":
                        # On affiche la photo de la preuve encadré en bleu à droite de l'écran un peu vers le haut
                        img_box = pygame.Rect(self.w - 310, 100, 230, 230)
                    else: 
                        # On affiche la photo de la preuve encadré en bleu à gauche de l'écran un peu vers le haut
                        img_box = pygame.Rect(80, 100, 230, 230)
                    
                    pygame.draw.rect(self.screen, pygame.Color(250, 250, 250), img_box, border_radius=6)
                    pygame.draw.rect(self.screen, pygame.Color(30, 90, 150), img_box, 6, border_radius=6)
                    asset = self.get_img(img)
                    if asset:
                        surf = asset.get() if isinstance(asset, AnimatedGIF) else asset
                        if surf:
