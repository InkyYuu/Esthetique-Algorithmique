...
class Game:
    ...
    def run(self):
        ...
        while running and self.lifes > 0:
            ...
            if node.scene.extraElements:
                for elements in node.scene.extraElements:
                    for element_name, element_value in elements.items():
                        if element_name == "add_evidence" and self.courtRecord.has_evidence(element_value) == False:
                            self.courtRecord.add_evidence(element_value)
            ...
            if node.scene.extraElements:
                ui.render_extra_elements(node.scene.extraElements, node.scene)
